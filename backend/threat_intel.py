...
def _fingerprint(alerts: list) -> str:
    ...
    detectors = sorted({sig.get("detector", "") for a in alerts for sig in a.get("signals", [])})
...
def generate_threat_intel(
    ...
    return {
        ...
        "global_detectors": dict(Counter(
            sig.get("detector", "") for a in alerts for sig in a.get("signals", [])
        ).most_common(10)),
    }

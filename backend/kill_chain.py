...
KILL_CHAIN_PHASES = [
    ...
    },
    {
        ...
        "detectors": [
            "missing_token", "jwt_alg_none", "jwt_alg_confusion", "jwt_bad_signature",
            "jwt_expired", "jwt_bad_issuer", "jwt_bad_audience", "jwt_malformed",
            "jwt_no_expiry", "jwt_not_yet_valid", "jwt_invalid", "jwt_revoked",
        ],
        "icon": "key",
    },
    {
...
def reconstruct_kill_chains(
    ...
    for subject, subject_alerts in by_subject.items():
        sorted_alerts = sorted(subject_alerts, key=lambda a: a.get("time", ""))
        ...
        all_timestamps = [a.get("time", "") for a in sorted_alerts if a.get("time")]

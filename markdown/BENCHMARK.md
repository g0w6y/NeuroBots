...
**Note the Redis/PostgreSQL rows below say "disconnected (in-memory fallback
active)"** - these numbers are the cheap-path case, not the real-Redis case.
See `backend/PERFORMANCE.md`'s "With real Redis attached" section for what
changes (materially) once both are actually connected.
